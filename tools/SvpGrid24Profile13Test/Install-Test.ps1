...
$backupRoot = Join-Path $env:ProgramData 'MPCVR-SVP4-Grid24-Test-Backup'
...
try {
    ...
    if ($Restore) {
        if (-not (Test-Path -LiteralPath $backupRoot -PathType Container)) {
            throw "No pre-test backup exists at $backupRoot"
        }
        ...
        Write-Host
        Write-Host 'Pre-grid24-test MPC Video Renderer binaries restored successfully.' -ForegroundColor Green
        Finish 0
    }
    ...
    # Preserve exactly what K-Lite had installed before this test package was
    # first used. Later reruns keep the same rollback point.
    if (-not (Test-Path -LiteralPath $backupRoot -PathType Container)) {
        New-Item -ItemType Directory -Path $backupRoot -Force | Out-Null
        $manifest = @(
            'MPCVR SVP4 grid24 test pre-install backup'
    ...
    foreach ($fileName in $targets.Keys) {
        ...
        $destination = $targets[$fileName]
        Write-Host "Installing SVP4 grid24 profile13 TEST $fileName..."
    ...
    Write-Host
    Write-Host 'MPCVR SVP4 grid24 profile13 TEST installed successfully.' -ForegroundColor Green
    Write-Host 'No registration changes were made. Restart MPC-HC and test normally.'
    Write-Host 'Use Restore-Previous.cmd to restore the binaries saved before this test.'
    Finish 0
}
catch {
    Write-Host
    Write-Host "SVP4 grid24 profile13 test installer failed: $($_.Exception.Message)" -ForegroundColor Red
    Finish 1
}
