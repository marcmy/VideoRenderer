...
@njit
def splat_direct(flow,err,qual,valid,wscale):
  ...
  for x in range(w):
   ...
   for oy in range(-1,2):
    ...
    for ox in range(-1,2):
     ...
     ddx=xx-tx;ddy=yy-ty;k=math.exp(-(ddx*ddx+ddy*ddy)/den)*conf;wq=int(math.floor(k*wscale+.5))
     if wq<=0:continue
     vx=dx*wq;vy=dy*wq;cxq=int(math.floor(vx+.5));cyq=int(math.floor(vy+.5));sx[yy,xx]+=cxq;sy[yy,xx]+=cyq;ws[yy,xx]+=wq
...
def process(ds):
 ...
 for wscl in [60]:
  da,wa=splat_direct(b,be,qb,be<=20,wscl);db,wb=splat_direct(f,fe,qf,fe<=20,wscl);O,S,a=finish(da,wa,db,wb,mass(wscl));r['schemes'][str(wscl)]={'mass':mass(wscl),'out':met(O,G),'vsFloat':diff(O,F),'splat':diff(S,Sf),'alphaMAD':float(np.mean(np.abs(a-af))),'maxWeightSum':int(max(wa.max(),wb.max()))}
