...
def metric(O,G):
 d=np.mean(np.abs(O-G),2); return [float(100*np.mean(d>1/255)),float(100*np.mean(d>4/255)),float(100*np.mean(d>8/255)),float(d.mean())]
def one(ds):
 d=Path(ds);A=load_rgb(d,'frame-A.bmp');B=load_rgb(d,'frame-B.bmp');G=load_rgb(d,'midpoint-current.bmp');T=.5*(A+B);f=load_flow(d,'flow-forward-B-to-A-s10.5.bin');b=load_flow(d,'flow-backward-A-to-B-s10.5.bin');fe,be=consistency_fields(f,b);q,occ,_=q_map(A,B,f,b);qf,qb=directional_q(A,B,f,b);_,_,cut=scene_stats(A,B);H,W=A.shape[:2];yy,xx=np.mgrid[0:H,0:W].astype(np.float32);sig=1.25;rad=3;ks=sum(math.exp(-(x*x+y*y)/(2*sig*sig)) for y in range(-rad,rad+1) for x in range(-rad,rad+1));da,wa=splat_q(b,be,qb,be<=20,sig,rad,1200);db,wb=splat_q(f,fe,qf,fe<=20,sig,rad,1200);CA=cv2.resize(np.clip(wa/ks,0,1),(W,H));CB=cv2.resize(np.clip(wb/ks,0,1),(W,H));DA=cv2.resize(da,(W,H));DB=cv2.resize(db,(W,H));AW=remap(A,xx+DA[...,0],yy+DA[...,1]);BW=remap(B,xx+DB[...,0],yy+DB[...,1]);aa=np.maximum(CA,1e-8)**3;bb=np.maximum(CB,1e-8)**3;den=aa+bb;S=np.where((den>1e-7)[...,None],(AW*aa[...,None]+BW*bb[...,None])/np.maximum(den[...,None],1e-7),T);M=np.median(np.stack([G,S,T],0),0);Q=cv2.resize(q,(W,H));support=np.maximum(CA,CB);sg=smoothstep(.03,.22,support);fg=0 if cut else float(smoothstep(15,25,np.array(occ)));out={'name':d.name,'field':occ,'fg':fg,'ramps':{}}
 for lo,hi in [(800,2000),(900,2100),(1000,2200),(1100,2300),(1200,2400),(1000,2400),(1200,2600)]:
  local=smoothstep(lo,hi,Q)*sg;local=cv2.GaussianBlur(local.astype(np.float32),(0,0),.8);alpha=np.minimum(local,.6)*fg;O=G*(1-alpha[...,None])+M*alpha[...,None];out['ramps'][f'{lo}-{hi}']={'metric':metric(O,G),'alpha':float(alpha.mean()),'a10':float(100*np.mean(alpha>.1))}
 return out
