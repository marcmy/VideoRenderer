...
OUT=Path('/mnt/data/vr_work/splat_field_ramp');OUT.mkdir(exist_ok=True)
...
def process(ds):
 ...
 da,wa=splat_q(b,be,qb,be<=20,sig,rad,1200);db,wb=splat_q(f,fe,qf,fe<=20,sig,rad,1200);CA=cv2.resize(np.clip(wa/ks,0,1),(W,H));CB=cv2.resize(np.clip(wb/ks,0,1),(W,H));DA=cv2.resize(da,(W,H));DB=cv2.resize(db,(W,H));AW=remap(A,xx+DA[...,0],yy+DA[...,1]);BW=remap(B,xx+DB[...,0],yy+DB[...,1]);aa=np.maximum(CA,1e-8)**3;bb=np.maximum(CB,1e-8)**3;den=aa+bb;S=np.where((den>1e-7)[...,None],(AW*aa[...,None]+BW*bb[...,None])/np.maximum(den[...,None],1e-7),T);M=np.median(np.stack([G,S,T],0),0).astype(np.float32);Q=cv2.resize(q,(W,H));support=np.maximum(CA,CB);local=smoothstep(1000,2200,Q)*smoothstep(.03,.22,support);local=cv2.GaussianBlur(local.astype(np.float32),(0,0),.8);local=np.minimum(local,.6)
 ramps={'HARD20':1.0 if occ>=20 else 0.0,'R15_25':float(smoothstep(15,25,np.array(occ))),'R17_27':float(smoothstep(17,27,np.array(occ))),'R18_28':float(smoothstep(18,28,np.array(occ))),'R20_30':float(smoothstep(20,30,np.array(occ)))}
 if cut:ramps={k:0.0 for k in ramps}
 res={'name':n,'field':occ,'cut':bool(cut),'variants':{}}
 for k,g in ramps.items():
  alpha=local*g;O=G*(1-alpha[...,None])+M*alpha[...,None];res['variants'][k]={'g':g,'alpha':float(alpha.mean()),**metric(O,G)};Image.fromarray(to8(O)).save(OUT/f'{n}_{k}.png')
 print(json.dumps(res),flush=True);return res
if __name__=='__main__':print(json.dumps([process(x) for x in sys.argv[1:]],indent=2))
