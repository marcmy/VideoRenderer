...
def patch_runtime() -> None:
    ...
    start_marker = "    int Interpolate(const MpcvrRifeRequest& request, MpcvrRifeStats& stats)\n"
    end_marker = "\n\nprivate:\n"
    start = text.index(start_marker)
    end = text.index(end_marker, start)

    replacement = r'''    int Interpolate(const MpcvrRifeRequest& request, MpcvrRifeStats& stats)
    {
...
def patch_kernel() -> None:
    path = Path("tools/RifeTensorRTRuntime/RifeKernels.cu")
    text = path.read_text(encoding="utf-8")

    start_marker = "template <typename T>\n__global__ void PackInputKernel("
    end_marker = "template <typename T>\n__global__ void WriteOutputKernel("
    start = text.index(start_marker)
    end = text.index(end_marker, start)

    replacement = r'''template <typename T>
__global__ void PackInputKernel(
