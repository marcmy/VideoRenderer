...
$backupRoot = Join-Path $env:ProgramData 'MPCVR-COV-FB55-Backup'
...
try {
    ...
    if ($Restore) {
        if (-not (Test-Path -LiteralPath $backupRoot -PathType Container)) {
            throw "No COV-FB55 backup exists at $backupRoot"
        }
        ...
        Write-Host
        Write-Host 'Pre-COV-FB55 MPC Video Renderer binaries restored successfully.' -ForegroundColor Green
        Finish 0
    }
    ...
    # Preserve the renderer that was installed before the first COV-FB55 test.
    # Never overwrite this backup on later experimental iterations.
    if (-not (Test-Path -LiteralPath $backupRoot -PathType Container)) {
        New-Item -ItemType Directory -Path $backupRoot -Force | Out-Null
        $manifest = @(
            'MPCVR COV-FB55 pre-test backup'
    ...
    Write-Host
    Write-Host 'MPCVR COV-FB55 experimental build installed successfully.' -ForegroundColor Green
    ...
    Write-Host
    Write-Host "COV-FB55 installer failed: $($_.Exception.Message)" -ForegroundColor Red
    Finish 1
}
